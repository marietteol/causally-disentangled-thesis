...
from config import SAMPLE_RATE, WIN_LENGTH, HOP_LENGTH, N_MELS

class SimCLRAudioAugment(nn.Module):
    def __init__(self, noise_std=0.005, gain_range=(0.8, 1.2), p_noise=0.5, p_gain=0.5):
        super().__init__()
...
class SimCLRCommonVoiceDataset(Dataset):
    def __init__(self, df, clips_dir, split='train', sample_rate=SAMPLE_RATE,
    ...
    def __getitem__(self, idx):
        ...
        if self.return_metadata:
            return s1.squeeze(0), s2.squeeze(0), row['speaker_id'], row['gender'], row['age_group'], row['accent_group']
        else:
